...
\item type resolver method (all types in Mini-Java can be resolved from the global scope)
\item a method to resolve the surrounding class scope for any ISyntaxTreeNode (if it exists)
\end{itemize}
...
In addition to MethodSymbols and UserDefinedTypeSymbols, there are other IScopes that are not Symbols, namely GlobalScope (an ITypeScope) and LocalScope (an IVariableScope), used for block statements, bodies of conditional statements etc.
