...
Boolean operators require a bit more effort. The \verb,not, opcode offered by CIL is a bitwise not operation. Because the CIL needs to represent boolean values as 32-bit integers (boolean literals are loaded using the opcodes \verb,ldc.i4.0, and \verb,ldc.i4.1,), this cannot be used as a boolean not operation. I have used the same strategy as used by the C$^\sharp$ compiler: emitting an \verb,ldc.i4.0, load opcode and an \verb,ceq, opcode, effectively comparing the boolean value on the stack with false, which results in the same truth table as the boolean not operator.

The CIL does offer opcodes \verb,and, and \verb,or, but in practice these cannot be used because boolean operators need to be short-circuited. This requires jump labels. The implementation of operator short-circuiting is best explained with an example. As an example, the following MiniJava code (a part of the test file \verb,shortcircuit.mjava, that can be found under \verb,testcode/, in the code directory):
\begin{verbatim}
...
The MiniJava compiler produces the following CIL code for the if statement condition:
\begin{verbatim}
// First operand:
IL_0006: ldc.i4.s 20
IL_0008: ldc.i4.2
...
This is similar to what the C$^\sharp$ compiler produces for boolean binary operators with the exception that it inverts some of the boolean values, but this has no effect on the functionality.
